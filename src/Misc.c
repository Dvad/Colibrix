...
int us, dt; // Variables pr�d�clar�s pour �viter l'allocation uniquement
...
int UMisc_GetTimerUs() {
	...
	gettimeofday(&tv, NULL);
	us = tv.tv_usec; // Renvoie les us de la seconde en cours (retombe donc � z�ro toutes les secondes !
	dt = us - lastUsTimer;
	lastUsTimer = us;
	if (dt < 0) {
...
void Misc_WaitEnter() {
	printf("Appuyer sur entr�e pour poursuivre...\n");
	fflush(stdout);
...
// Fonction kbhit()
//
// Retourne 1 si des donn�es sont dispo sur le stream
// Retourne 0 sinon
//
...
void Misc_PrintHelp() {
	...
	printf("  Gestion d'�tat:\n");
	printf("    w: Clear\n");
	printf("    x: Initialisation\n");
	printf("    c: Rotation\n");
	printf("    v: D�collage\n");
	printf("    b: Atterissage\n");
	printf("  R�glages manuels:\n");
	printf("    z/s: Tangage\n");
	printf("    q/d: Roulis\n");
	printf("    a/e: Lacet\n");
	printf("    r/f: Altitude\n");
	printf("    p/m: R�glage de Vequi\n");
	printf("    $/*: R�glage de l'altitude apr�s d�collage\n");
	printf("  Divers:\n");
	printf("    y: Active/d�sactive le sonar\n");
	printf("    u: Buffer centrale\n");
	printf("    j: Mode I2C\n");
	printf("    n: Affichage\n");
	printf("    i: Infos fr�quences\n");
	printf("    l: Gains PID\n");
	printf("    h: Affiche l'aide\n");
	printf("    k: Termine le programme\n");
	printf("   [espace]: Arret d'urgence\n");
	printf("   set {l/r/t/a}{p/i/d}: R�glage d'un gain\n");
...
/*
 ...
 //
 // D�monstration de comment transformer un double en octets
 // puis a nouveau en double grace a memcpy et des pointeurs
 //
 double d = 1.0; // Le double de d�part
