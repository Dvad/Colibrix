...
unsigned char commandePulseSonar[5];
...
int I2C_Envoyer_Pulse_Sonar(){
	...
	commandePulseSonar[0]=0x0; //registre
	commandePulseSonar[1]=0x55; //Pour une réponse en cm voir datasheet
	commandePulseSonar[2]=GAIN_MAX_SONAR;
	commandePulseSonar[3]=RANGE_SONAR_REGISTER;
	ret=write(fdc,commandePulseSonar,4);
	if (ret==4)return 0 ;
	else{
		printf("Erreur communication Sonar : %s",strerror(errno));
		return ret;
	}
...
int I2C_Envoyer_Lecture_Sonar(){
	//
	//On écrit pour sélectionner le registre
	//
