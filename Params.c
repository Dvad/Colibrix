...
void Params_Initialise()
{
	...
	//
	I_ROUL = 0.038F; // Moment d'inertie en roulis   // Valeur à recalculer
	I_TANG = 0.038F; // Moment d'inertie en tangage  // Valeur à recalculer
	I_LACT = 0.068F; // Moment d'inertie en lacet    // Valeur à recalculer
	MASSE_TOTALE = 1.000F;
	...
	// représente le temps d'anticipation total du système
	//
	...
	//   Ces paramètres peuvent etre sérrés sur le banc d'essai,
	...
	//
	// Les paramètres suivant saturent la consigne envoyé par le PID
	//   Il doivent donc etre:
	//     - lache en RT
	//     - moyen en A
	//     - rel. sérré en L
	UA_MAX = GAINS_PRO[3] * CM_EN_M * 60.0F; // En centimètres
	UA_MIN = - UA_MAX;
